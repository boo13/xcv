...
def main_input(
    ...
    # local imports
    from xcv import __main__.single_btn_press
    import xcv.settings
    # import __main__
    ...
    xcv.settings.Settings.verbose = verbose
    xcv.settings.Settings.port = port
    xcv.settings.Settings.timerFlag = count

    if dryrun:
        print("WIP feature")
    elif push:
        xcv.__main__.single_btn_press(push)
    elif autopilot:
        xcv.__main__.start_btn_press_sequence()
    else:
        click.echo(
